...
@section('konten')
    <section class="container-edit_profile rounded">
        ...
        <article class="header-edit_profile">
            <a href="{{ route('show', $password->id) }}" class="back-profile" style="padding: 30px;"><i
                    class="fa-solid fa-arrow-left icon_back-profile"></i></a>
            <h1 class="text-white fs-2 fw-bold m-0 px-5 py-5">{{ $title }}</h1>
        </article>
                        ...
                        <div class="col-lg-12 row py-4 mx-0">
                            ...
                            <div class="col-lg-12 p-0 d-flex justify-content-between password-toggle gap-3">
                                <input class=" w-100 m-0 px-3 py-lg-3 py-md-3 rounded border border-2 rounded"
                                    type="password" placeholder="Masukkan Password Lama" id="current_password"
                        ...
                        <div class="col-lg-12 row py-4 mx-0">
                            ...
                            <div class="col-lg-12 p-0 d-flex justify-content-between mb-2 password-toggle gap-3">
                                <input id="passwordInput" class="w-100 m-0 px-3 py-lg-3 py-md-3 border border-2 rounded"
                                    type="password" placeholder="Masukkan Password Baru" id="password" name="password">
                                <button
                                    class="toggle-password btn btn-hides rounded py-2 px-lg-3 px-3 d-flex align-items-center border border-2 rounded"
                                    style="box-shadow: 4px 4px 8px rgba(0, 0, 0, 0.25);" type="button" id="showPassword"><i
                                        class="fa-regular fa-eye text-muted m-0 p-0 "></i></button>
                        ...
                        <div class="col-lg-12 row py-4 mx-0">
                            ...
                            <div class="col-lg-12 p-0 d-flex justify-content-between password-toggle gap-3">
                                <input class=" w-100 m-0 px-3 py-lg-3 py-md-3 border border-2 rounded" type="password"
                                    placeholder="Ulangi Password Baru" id="confirm_password" name="confirm_password">
                                <button
                                    class="toggle-password btn btn-hides rounded py-2 px-lg-3 px-3 d-flex align-items-center border border-2 rounded"
                                    style="box-shadow: 4px 4px 8px rgba(0, 0, 0, 0.25);" type="button"
                                    id="showPassword"><i class="fa-regular fa-eye text-muted m-0 p-0 "></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="d-flex gap-5 btn-edit_save my-5">
                    <a href="{{ route('show', $password->id) }}" class="btn btn_batal-edit">Batal</a>
                    <button class="btn btn_simpan-edit" id="simpan" data-bs-target="#actionModalTambah"
                        data-bs-toggle="modal" data-bs-dismiss="modal" onclick="ubahPassword()">Simpan</button>
                </div>
            </article>
        </form>
    ...
    <script>
        document.querySelector('.btn_batal-edit').addEventListener('click', function() {
            window.history.back();
        });
        ...
        function ubahPassword() {
            ...
            if (!form.checkValidity()) {
                document.getElementById("alert-error").show();
                return false;
            } else {
                form.submit();
                document.getElementById("alert-success").show();
            }
        }
    </script>
    {{-- <script>
