                            ...
                            @foreach ($data_barang as $data_brg)
                                <tr>
                                    ...
                                    <td class="pb-0 pt-3 d-flex  justify-content-evenly gap-md-2 mx-auto">
                                        {{-- <a href=" {{ route('barangs.update', $data_brg->id) }}"> --}}
                                            ...
                                            {{-- <button class="btn btn-sm btn-orange px-lg-4 text-center rounded-2 fw-medium"
                                                ...
                                                onclick="populateEditForm('{{ route('cs.dataBarang.update', $data_brg->id) }}')"
                                            ...
                                            <button class="btn btn-sm btn-orange px-lg-4 text-center rounded-2 fw-medium"
                                                data-bs-toggle="modal" data-bs-target="#editBarangModal"
                                                data-id="{{ $data_brg->id }}" onclick="changeURLAndShowModal({{ $data_brg->id }})"
                                                style="font-size: 12px;">
                                                <span id="icon" class="fa-solid fa-pen"></span>
                                                <span id="text">Edit</span>
                                            </button>
...
@else
    ...
    {{-- Modal Edit Barang --}}
    <div class="modal fade" id="editBarangModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header border-bottom-0 m-3 py-0 ">
    ...
    <script>
        ...
        function editBarangg(id) {
            // event.preventDefault();
            // $.ajax({
            //     url: '/cs/dataBarang/update/' + id,
            //     type: 'GET', // Menggunakan metode PATCH
        ...
        function changeURLAndShowModal(id) {
            history.pushState({}, null, "/cs/dataBarang/update/" + id);
            var modal = document.getElementById("editBarangModal").show();
        }
